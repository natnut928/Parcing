...
# выводит на экран вакансии с заработной платой больше введённой суммы (необходимо анализировать оба поля зарплаты).

salary_filter = int(input('Больше какой суммы: '))
for salary in collection_db.find({"$or": [{'salary_min': {"$gt": salary_filter}, 'salary_max':{"$gt": salary_filter}}]}):
    pprint(salary)
